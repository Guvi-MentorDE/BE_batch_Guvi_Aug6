...
WITH salary_after_tax_CTE AS
(
    SELECT *, salary - ((0/100)*salary) AS salary_after_tax 
    FROM
    (
    ...
    UNION
    SELECT *, salary - ((24/100)*salary) AS salary_after_tax 
    FROM
    (
        SELECT *, MAX(salary) OVER(PARTITION BY company_id) AS max_salary 
        FROM salaries
    ) max_salary_tab
    WHERE max_salary BETWEEN 1000 AND 10000
    UNION
    SELECT *, salary - ((49/100)*salary) AS salary_after_tax 
    FROM
    (
